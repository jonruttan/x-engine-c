/*
 * # Unit Tests: *x-type/int*
 */

#define TEST_RUNNER_OVERHEAD
#include "test-runner.h"

/* We need the GC structures for cleanup. */
#ifndef X_GC
#define X_GC
#endif /* X_GC */

#include "ext/x-expr/tests/src/test-helper-system.c"

#include "ext/x-expr/src/x-sys.c"
#include "ext/x-expr/src/x-stdlib.c"
#include "ext/x-expr/src/x-lib.c"
#include "ext/x-expr/src/x.c"
#include "ext/x-expr/src/x-obj.c"
#include "src/x-alist.c"
#include "ext/x-expr/src/x-base.c"
#define STUB_X_EVAL
#include "src/x-eval.c"
#include "ext/x-expr/src/x-heap.c"
#include "src/x-type.c"
#include "src/x-type/prim.c"
#include "src/x-type/char.c"
#include "src/x-type/err.c"
#include "src/x-token/sexp/char.c"
#include "src/x-type/buffer.c"
#include "src/x-type/int.c"
#include "src/x-token/sexp/int.c"
#include "src/x-type/str.c"
#include "src/x-token/sexp/str.c"

#define STUB_X_PRIM
#define STUB_X_PROCEDURE
#define STUB_X_OPERATIVE
#define STUB_X_EVAL
#define STUB_X_TOKEN
#define STUB_X_HEAP
#define STUB_X_OBJ_OBJ
#define STUB_X_SYMBOL
#define STUB_X_PRIM_REGISTER
#define STUB_X_PRIM_SHADOW
#define STUB_X_PROCEDURE_APPLY
#include "helper-stubs.c"


/*
 * ## Test Overhead
 */

static void _setup(void)
{
	helper_set_alloc(MEM_GUARANTEED);
	helper_sys_funcs.exit = mock_exit;
	helper_sys_funcs.malloc = helper_malloc;
	helper_sys_funcs.free = helper_free;
}

static void _teardown(void)
{
}

void test_cleanup(x_obj_t *p_base)
{
	x_obj_t *p_gc = p_base, *p_tmp;

	while (p_gc) {
		p_tmp = x_obj_heap(p_gc);
		x_sys_free(p_gc);
		p_gc = p_tmp;
	}
}

/*
 * ## Test Runners
 */

#define X_TEST_INT_VALUE		0xa5

#define nil			NULL
#define pair(X,Y)	(x_mkspair(p_base, X_OBJ_FLAG_NONE, (X), (Y)))
#define atom(X)		(x_mksatom(p_base, X_OBJ_FLAG_NONE, (X)))

static char *test_obj_type_isint(void)
{
	x_obj_t *p_base, *p_obj;

	p_base = x_eval_make(NULL, NULL);

	p_obj = x_mkint(p_base, 0);
	_it_should("return true when object is an int",
		1 == x_obj_type_isint(p_base, p_obj)
	);

	p_obj = x_mksatom(p_base, X_OBJ_FLAG_NONE, 0);
	_it_should("return false when object is not an int",
		0 == x_obj_type_isint(p_base, p_obj)
	);

	test_cleanup(p_base);

	return NULL;
}

static char *test_intval(void)
{
	x_obj_t *p_obj;
	x_int_t c, value = X_TEST_INT_VALUE;

	p_obj = x_mksatom(NULL, X_OBJ_FLAG_NONE, value);

	c = x_intval(p_obj);
	_it_should("return the Integer's value", value == c);

	x_sys_free(p_obj);

	return NULL;
}

static char *test_mkint(void)
{
	x_obj_t *p_obj, *p_base;
	x_int_t i = rand();

	p_base = x_eval_make(NULL, NULL);

	p_obj = x_mkint(p_base, i);
	_it_should("make an Integer object and set its value",
		p_obj != NULL
		&& X_OBJ_FLAG_NONE == x_obj_flags(p_obj)
		&& i == x_intval(p_obj)
	);

	x_sys_free(p_obj);


	p_base = x_eval_make(NULL, NULL);
	p_obj = x_mkint(p_base, i);
	_it_should("make an Integer object, attach it to the Base object, and set its value",
		p_obj != NULL
		&& X_OBJ_FLAG_NONE == x_obj_flags(p_obj)
		&& i == x_intval(p_obj)
	);

	test_cleanup(p_base);


	return NULL;
}

static char *test_mkfint(void)
{
	x_obj_t *p_obj, *p_base;
	x_int_t i = rand();
	x_obj_flag_t flags = rand();

	p_base = x_eval_make(NULL, NULL);

	p_obj = x_mkfint(p_base, flags, i);
	_it_should("make an Integer object and set its value",
		p_obj != NULL
		&& flags == (x_obj_flag_t)x_obj_flags(p_obj)
		&& i == x_intval(p_obj)
	);

	x_sys_free(p_obj);


	p_base = x_eval_make(NULL, NULL);
	p_obj = x_mkfint(p_base, flags, i);
	_it_should("make an Integer object, attach it to the Base object, and set its value",
		p_obj != NULL
		&& flags == (x_obj_flag_t)x_obj_flags(p_obj)
		&& i == x_intval(p_obj)
	);

	test_cleanup(p_base);


	return NULL;
}

static char *test_make_int(void)
{
	x_obj_t *p_obj, *p_base;
	x_int_t i = rand();
	x_obj_flag_t flags = rand();

	p_base = x_eval_make(NULL, NULL);

	p_obj = x_make_int(p_base, flags, i);
	_it_should("make an Integer object and set its value",
		p_obj != NULL
		&& flags == (x_obj_flag_t)x_obj_flags(p_obj)
		&& i == x_intval(p_obj)
	);

	x_sys_free(p_obj);


	p_base = x_eval_make(NULL, NULL);
	p_obj = x_make_int(p_base, flags, i);
	_it_should("make an Integer object, attach it to the Base object, and set its value",
		p_obj != NULL
		&& flags == (x_obj_flag_t)x_obj_flags(p_obj)
		&& i == x_intval(p_obj)
	);

	test_cleanup(p_base);


	return NULL;
}

static char *test_type_int_register(void)
{
	x_obj_t *p_base, *p_type;

	p_base = x_eval_make(NULL, NULL);

	p_type = x_type_int_register(p_base, p_base);
	_it_should("return the Integer type object",
		0 == x_lib_strcmp(X_TYPE_INT_NAME, x_strval(x_type_field_name(p_type)))
	);
	_it_should("add the Integer type to the Type alist",
		p_type == x_restobj(x_firstobj(x_firstobj(x_eval_field_type_alist(p_base))))
	);

	test_cleanup(p_base);

	return NULL;
}

static char *test_type_int_struct(void)
{
	x_obj_t *p_base, *p_type;

	helper_alloc_reset();

	p_base = x_eval_make(NULL, NULL);
	p_type = x_type_int_struct(p_base, p_base);
	_it_should("return Integer Type list",
		! x_obj_isnil(p_base, p_type)
		&& 0 == strcmp(X_TYPE_INT_NAME, x_atomstr(x_type_field_name(p_type)))
	);

	_it_should("contain the Name object",
		x_type_int_name == x_type_field_name(p_type)
	);

	_it_should("set the Data object to nil",
		NULL == x_type_field_data(p_type)
	);

	_it_should("set the Make primitive",
		x_type_int_make == x_primval(x_type_field_make(p_type))
	);

	_it_should("not set the Free primitive",
		NULL == x_type_field_free(p_type)
	);

	_it_should("not set the Clone primitive",
		NULL == x_type_field_clone(p_type)
	);

	_it_should("not set the Units primitive",
		NULL == x_type_field_units(p_type)
	);

	_it_should("not set the Length primitive",
		NULL == x_type_field_length(p_type)
	);

	_it_should("not set the Call primitive",
		NULL == x_type_field_call(p_type)
	);

	_it_should("not set the Eval primitive",
		NULL == x_type_field_eval(p_type)
	);

	_it_should("not set the From alist",
		NULL == x_type_field_from(p_type)
	);

	_it_should("not set the To alist",
		NULL == x_type_field_to(p_type)
	);

	_it_should("set the Analyse primitive",
		x_sexp_int_analyse_sign_prim == x_type_field_analyse(p_type)
	);

	_it_should("not set the Delimit primitive",
		NULL == x_type_field_delimit(p_type)
	);

	_it_should("leaves the Write cell empty (the printer is x-lang)",
		x_type_field_write(p_type) == NULL
	);

	test_cleanup(p_base);

	return NULL;
}

static char *test_base_alist_assoc(void)
{
	x_obj_t *p_base, *p_type;

	helper_alloc_reset();

	p_base = x_eval_make(NULL, NULL);
	x_eval_type_alist_extend(p_base, x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mkspair(p_base, X_OBJ_FLAG_NONE, x_type_int_name, NULL), atom(1)));

	p_type = x_eval_type_alist_assoc(p_base, x_mkspair(p_base, X_OBJ_FLAG_NONE, x_type_int_name, NULL));
	_it_should("find the type in the Type alist and return its properties",
		x_type_int_name == x_type_field_name(p_type)
	);

	return NULL;
}

static char *test_type_int_make(void)
{
	x_obj_t *p_base, *p_args, *p_int, *p_obj[2];
	x_int_t value = X_TEST_INT_VALUE;

	helper_alloc_reset();

	/* NULL p_base object */
	p_int = x_mksatom(NULL, X_OBJ_FLAG_NONE, value);
	p_args = x_mkspair(NULL, X_OBJ_FLAG_NONE, p_int, NULL);

	p_obj[0] = x_type_int_make(NULL, p_args);
	_it_should("make  object",
		p_obj[0] != NULL
		&& value == x_intval(p_obj[0])
	);

	p_obj[1] = x_type_int_make(NULL, p_args);
	_it_should("make a second Integer object",
		p_obj[1] != NULL
		&& value == x_intval(p_obj[1])
	);

	_it_should("have returned a different Type object for both objects",
		x_obj_type(p_obj[0]) != x_obj_type(p_obj[1])
	);

	x_sys_free(p_obj[1]);
	x_sys_free(p_obj[0]);
	x_sys_free(p_args);
	x_sys_free(p_int);


	helper_alloc_reset();

	/* Empty p_base object */
	p_base = x_eval_make(NULL, NULL);
	p_int = x_mksatom(p_base, X_OBJ_FLAG_NONE, value);
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, p_int, NULL);

	p_obj[0] = x_type_int_make(p_base, p_args);
	_it_should("make  object",
		p_obj[0] != NULL
		&& value == x_intval(p_obj[0])
	);

	p_obj[1] = x_type_int_make(p_base, p_args);
	_it_should("make a second Integer object",
		p_obj[1] != NULL
		&& value == x_intval(p_obj[1])
	);

	_it_should("have returned a different Type object for both objects",
		x_obj_type(p_obj[0]) == x_obj_type(p_obj[1])
	);

	test_cleanup(p_base);


	helper_alloc_reset();

	/* With p_base object */
	p_base = x_eval_make(NULL, NULL);
	p_int = x_mksatom(p_base, X_OBJ_FLAG_NONE, value);
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, p_int, NULL);

	p_obj[0] = x_type_int_make(p_base, p_args);
	_it_should("make  object with a base object",
		p_obj[0] != NULL
	);

	p_obj[1] = x_type_int_make(p_base, p_args);
	_it_should("make a second Integer object a base object",
		p_obj[1] != NULL
	);
	_it_should("have returned the same type object for both objects",
		x_obj_type(p_obj[0]) == x_obj_type(p_obj[1])
	);

	test_cleanup(p_base);


	return NULL;
}

static char *run_tests()
{
	_run_test(test_obj_type_isint);
	_run_test(test_intval);
	_run_test(test_mkint);
	_run_test(test_mkfint);
	_run_test(test_make_int);
	_run_test(test_type_int_register);
	_run_test(test_type_int_struct);
	_run_test(test_base_alist_assoc);
	_run_test(test_type_int_make);

	return NULL;
}
