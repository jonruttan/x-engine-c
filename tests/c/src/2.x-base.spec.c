...
/*	p_obj = x_eval_field_type_alist(p_base);
...
*/
	...
	p_obj = x_firstobj(x_eval_field_token_cache(p_base));
...
static char *test_base_type_alist_extend(void)
{
	...
	p_alist = x_eval_type_alist_extend(NULL, p_args);
	...
	p_alist = x_eval_type_alist_extend(p_base, p_type0);
	...
	p_alist = x_eval_type_alist_extend(p_base, p_type1);
...
static char *test_base_type_alist_assoc(void)
{
	...
	p_args = x_mkspair(NULL, X_OBJ_FLAG_NONE,
		x_mksatom(NULL, X_OBJ_FLAG_NONE, s[0]),
		NULL
	);

	p_obj[0] = x_eval_type_alist_assoc(NULL, p_args);
	_it_should("return NULL when base is NULL",
		NULL == p_obj[0]
	);

	p_base = x_eval_make(NULL, NULL);
	p_obj[0] = x_eval_type_alist_assoc(p_base, p_args);
	...
	p_name[0] = x_mksatom(p_base, X_OBJ_FLAG_NONE, s[0]);
	...
	x_eval_type_alist_extend(p_base, p_type[0]);
	x_eval_type_alist_extend(p_base, p_type[1]);
	...
	p_obj[1] = x_eval_type_alist_assoc(p_base, p_args);
...
static char *test_base_read(void)
{
	...
	p_args = x_mkspair(NULL, X_OBJ_FLAG_NONE,
		...
		x_mkspair(NULL, X_OBJ_FLAG_NONE, x_mksatom(NULL, X_OBJ_FLAG_NONE, 1), NULL)
	);
...
static char *test_base_write(void)
{
	...
	p_args = x_mkspair(NULL, X_OBJ_FLAG_NONE,
		x_mksatom(NULL, X_OBJ_FLAG_NONE, "01"),
		x_mkspair(NULL, X_OBJ_FLAG_NONE, x_mksatom(NULL, X_OBJ_FLAG_NONE, 2), NULL)
	);
...
static char *test_base_error_with_handler(void)
{
	...
	/* x_eval_error now stores message in a static atom (zero allocation) */
...
static char *test_base_write_buf(void)
{
	...
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE,
		...
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksatom(p_base, X_OBJ_FLAG_NONE, 5), NULL)
	);
	...
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE,
		x_mksatom(p_base, X_OBJ_FLAG_NONE, "!"),
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksatom(p_base, X_OBJ_FLAG_NONE, 1), NULL)
	);
	x_base_write(p_base, p_args);
...
static char *test_base_load(void)
{
	...
	p_base = x_eval_make(NULL, NULL);
	/* x_eval_load reads x_base_field_buffer but our stubbed x_token_read
	 * ignores it — just push a non-NULL dummy. */
	x_eval_buffer_push(p_base, x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL, NULL));
	...
	p_ret = x_eval_load(p_base, NULL);
	_it_should("return NULL for empty input", NULL == p_ret);

	/* Two tokens then NULL */
	tokens[0] = x_mksatom(p_base, X_OBJ_FLAG_NONE, 42);
	tokens[1] = x_mksatom(p_base, X_OBJ_FLAG_NONE, 99);
	tokens[2] = NULL;
