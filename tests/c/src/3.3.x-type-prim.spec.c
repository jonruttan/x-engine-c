...
static char *test_base_alist_assoc(void)
{
	...
	p_base = x_eval_make(NULL, NULL);
	x_eval_type_alist_extend(p_base, x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mkspair(p_base, X_OBJ_FLAG_NONE, x_type_prim_name, NULL), atom(1)));

	p_type = x_eval_type_alist_assoc(p_base, x_mkspair(p_base, X_OBJ_FLAG_NONE, x_type_prim_name, NULL));
...
static char *test_type_prim_make(void)
{
	...
	/* NULL p_base object */
	p_fn = x_mksatom(NULL, X_OBJ_FLAG_NONE, test_prim_fn);
	p_args = x_mkspair(NULL, X_OBJ_FLAG_NONE, p_fn, NULL);
...
static char *test_type_prim_call(void)
{
	...
	p_call = x_mksatom(NULL, X_OBJ_FLAG_NONE, test_prim_fn);
...
{
	x_obj_t *p_name = x_mksatom(p_base, X_OBJ_FLAG_NONE, name);
	...
	p_state = x_mkspair(p_base, X_OBJ_FLAG_SHARED, NULL,
		x_mkspair(p_base, X_OBJ_FLAG_SHARED, NULL,
			x_mkspair(p_base, X_OBJ_FLAG_SHARED, NULL, NULL)));
