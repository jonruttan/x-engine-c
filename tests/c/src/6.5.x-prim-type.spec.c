...
static char *test_type_typep(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "myint"), p_int);
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "inthandle"), p_int_handle);
...
static char *test_type_null_read_discard(void)
{
	...
	desc.p_name = x_mksatom(p_base, X_OBJ_FLAG_NONE, "DISCARD");
...
static char *test_type_make_type(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "name"), x_mkstr(p_base, "mytype"));
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "handlers"), p_handlers);

	/* (make-type name handlers) */
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL,
		x_mkspair(p_base, X_OBJ_FLAG_NONE,
		x_mksymbol(p_base, "name"),
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "handlers"), NULL)));
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "th"), p_name_handle);
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL,
		x_mkspair(p_base, X_OBJ_FLAG_NONE,
...
static char *test_type_make_type_with_handlers(void)
{
	...
	p_handlers = x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "eval"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "call"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "write"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "length"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "analyse"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "delimit"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "error"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "from"), p_fn),
		x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "to"), p_fn),
		NULL)))))))));

	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "name"), x_mkstr(p_base, "fulltype"));
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "hdlrs"), p_handlers);
...
static char *test_type_base_make_type(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "tgt"), p_target);
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "name"), x_mkstr(p_base, "tgttype"));
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "hdlrs"), NULL);
...
static char *test_type_make_base(void)
{
	...
	_it_should("make-base returns different base",
		p_new_base != p_base);
	_it_should("make-base has a root environment",
		x_eval_field_env(p_new_base) != NULL
		&& x_eval_field_env(p_new_base) == x_eval_field_env_root(p_new_base));
...
static char *test_type_base_eval(void)
{
	...
	x_env_bind(p_target, x_eval_field_env(p_target), p_sym, x_mksatom(p_target, X_OBJ_FLAG_NONE, (x_int_t)77));
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "expr"), p_sym);

	/* (base-eval tgt expr) -> evaluates xx in target -> 77 */
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL,
		x_mkspair(p_base, X_OBJ_FLAG_NONE,
		x_mksymbol(p_base, "tgt"),
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "expr"), NULL)));
...
static char *test_type_base_eval_error(void)
{
	 ...
	 * Bind it in calling base so the first eval resolves it to the symbol. */
	{
		...
		x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "expr"), p_unbound);
	}
...
static char *test_type_base_bind(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "tgt"), p_target);
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "nm"), x_mksymbol(p_base, "hello"));
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "val"), x_mksatom(p_base, X_OBJ_FLAG_NONE, (x_int_t)55));
...
static char *test_type_buffer_token(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "buf"), p_buffer);
...
static char *test_type_token_read_string(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "tb"), p_token_base);
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "s"), x_mkstr(p_base, "hello"));
	...
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL,
		...
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "s"), NULL)));
...
static char *test_type_convert(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "v"), p_int);
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "h"), p_int_handle);

	/* (convert v h) -> v (same type, short-circuit) */
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL,
		x_mkspair(p_base, X_OBJ_FLAG_NONE,
		x_mksymbol(p_base, "v"),
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "h"), NULL)));
	p_result = x_prim_convert(p_base, p_args);
	_it_should("convert short-circuits for same type",
		p_result == p_int);

	/* (convert nil h) -> nil */
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "v"), NULL);
	p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL,
		x_mkspair(p_base, X_OBJ_FLAG_NONE,
		x_mksymbol(p_base, "v"),
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "h"), NULL)));
	...
	/* (convert v bogus-handle) -> nil (no match) */
	{
		...
		x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "v"), p_int);
		x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "bh"), p_bogus);
		p_args = x_mkspair(p_base, X_OBJ_FLAG_NONE, NULL,
		x_mkspair(p_base, X_OBJ_FLAG_NONE,
			x_mksymbol(p_base, "v"),
			x_mkspair(p_base, X_OBJ_FLAG_NONE, x_mksymbol(p_base, "bh"), NULL)));
		p_result = x_prim_convert(p_base, p_args);
...
static char *test_type_convert_from_exact(void)
{
	...
	x_eval_type_alist_extend(p_base, p_src_type);
	...
	/* Bind to env */
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "v"), p_instance);
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "h"), p_tgt_handle);
...
static char *test_type_convert_wildcard(void)
{
	...
	p_from_alist = x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, x_eval_field_true(p_base), p_converter),
		NULL);

	/* Create target type with wildcard from alist */
	memset(&tgt_desc, 0, sizeof(tgt_desc));
	tgt_desc.p_name = x_mksatom(p_base, X_OBJ_FLAG_NONE, "WTGT");
	tgt_desc.p_units = (x_obj_t *)&x_type_units_pair_obj;
	tgt_desc.p_from = p_from_alist;
	p_tgt_type = x_type_struct_make(p_base, tgt_desc);
	x_eval_type_alist_extend(p_base, p_tgt_type);
...
static char *test_type_convert_to_alist(void)
{
	...
	memset(&tgt_desc, 0, sizeof(tgt_desc));
	tgt_desc.p_name = x_mksatom(p_base, X_OBJ_FLAG_NONE, "TTGT");
	tgt_desc.p_units = (x_obj_t *)&x_type_units_pair_obj;
	x_type_struct_make(p_base, tgt_desc);
	p_tgt_handle = tgt_desc.p_name;
	x_eval_type_alist_extend(p_base,
		x_type_struct_make(p_base, tgt_desc));

	/* Build to alist: ((tgt-handle . converter)) */
	p_to_alist = x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, p_tgt_handle, p_converter), NULL);

	/* Create source type with to alist */
	memset(&src_desc, 0, sizeof(src_desc));
	src_desc.p_name = x_mksatom(p_base, X_OBJ_FLAG_NONE, "TSRC");
	src_desc.p_units = (x_obj_t *)&x_type_units_pair_obj;
...
static char *test_type_convert_no_match(void)
{
	...
	p_converter = x_mkprim(p_base, test_convert_handler);
	p_bogus_key = x_mksatom(p_base, X_OBJ_FLAG_NONE, "BOGUS");

	/* Target type has from alist, but key doesn't match source */
	p_from_alist = x_mklist(p_base,
		x_mkspair(p_base, X_OBJ_FLAG_NONE, p_bogus_key, p_converter), NULL);
	...
	memset(&src_desc, 0, sizeof(src_desc));
	src_desc.p_name = x_mksatom(p_base, X_OBJ_FLAG_NONE, "NMSRC");
	src_desc.p_units = (x_obj_t *)&x_type_units_pair_obj;
	src_desc.p_to = p_to_alist;
	p_src_type = x_type_struct_make(p_base, src_desc);
	x_eval_type_alist_extend(p_base, p_src_type);
...
static char *test_type_token_read_string_tokens(void)
{
	...
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "tb"), p_token_base);
	x_env_bind(p_base, x_eval_field_env(p_base), x_mksymbol(p_base, "s"), x_mkstr(p_base, "(1)(2)"));
