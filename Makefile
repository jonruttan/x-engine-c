...
# The DECLARATION ratchet, and the one gate that is not self-contained: it
# needs an x-lang checkout, because x-engine.xon is generated by x-lang's
# tools/contract/gen-engine-xon.sh and producing it needs the language's
# feature vocabulary.  So it is deliberately NOT in `gates` and NOT in `test`,
# both of which must work on a bare clone with nothing beside it.
#
# What it buys: an edit to isa.x or claims.x without a regenerated
# declaration is caught HERE instead of downstream on x-lang's next pin bump.
# That has shipped twice -- 0.1.5 and 0.2.1, both stale digests, both found by
# the consumer -- and both would have failed this target.
#
# Finds a sibling x-lang checkout by default; CI clones one and passes
# X_LANG_DIR.
check-declaration: ## Diff x-engine.xon against what x-lang's generator produces
	sh tools/check/declaration.sh
